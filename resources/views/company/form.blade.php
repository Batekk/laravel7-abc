                        ...
                        <div class="form-group-lg">
                            ...
                            <p class='card-text'>
                                @foreach($company->users as $user)
                                    <a href="{{ route('users.edit', $user->_id) }}">{{ $user->name}}</a>@if($loop->count > 1 && !$loop->last)
                                        ,@endif
                                @endforeach
                            </p>
                        </div>
                        ...
                        <div class="form-group-lg">
                            ...
                            <p class='card-text'>
                                @foreach($company->category as $category)
                                    <a href="{{ route('category.edit', $category->_id) }}">{{ $category->name}}</a>@if($loop->count > 1 && !$loop->last)
                                        ,@endif
                                @endforeach
                            </p>
                        </div>
                            ...
                            <p class='card-text'>
                                @foreach($company->category as $category)
                                    @foreach($category->services as $service)
                                        <a href="{{ route('services.edit', $service->_id) }}">{{ $service->name}}</a>@if($loop->count > 1 && !$loop->last)
                                            ,@endif
                                    @endforeach
                                @endforeach
                            </p>
